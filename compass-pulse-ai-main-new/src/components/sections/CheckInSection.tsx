...
export function CheckInSection({
  ...
  // reply engine as the floating Work Buddy AI assistant (workBuddyAiReply.ts), just asked in-context here so
  // the manager doesn't have to leave the check-in flow to get it.
  const askWorkBuddyAi = async () => {
    ...
    logAiActivity({
      date: new Date().toISOString().slice(0, 10), kind: "prep_agent",
      summary: `Work Buddy AI asked for help with ${member.name}'s check-in: "${query.length > 60 ? `${query.slice(0, 60)}…` : query}"`,
        ...
        <div className="px-3.5 pb-3.5 space-y-3">
          {!logging ? (
            ...
            <div className="rounded-lg border border-primary/25 bg-primary/5 p-3 space-y-2.5">
              ...
              {/* Ask Work Buddy AI — on top of the top-5 auto-drafted points above, for anything else:
              ...
              <div className="border-t border-primary/15 pt-2">
                {!askOpen ? (
                  <button onClick={() => setAskOpen(true)} className="flex items-center gap-1 text-[11px] font-medium text-primary hover:underline">
                    <MessageCircle className="size-3" /> Need a hand? Ask Work Buddy AI
                  </button>
                ) : (
                  <div className="space-y-1.5">
                    <label className="text-[10px] text-muted-foreground">Ask Work Buddy AI — e.g. "how do I bring up a hard topic with {member.name.split(" ")[0]}?"</label>
                    <div className="flex gap-1.5">
                      <input
                        value={askQuery} onChange={e => setAskQuery(e.target.value)}
                        onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); void askWorkBuddyAi(); } }}
                      ...
                      />
                      <button onClick={() => void askWorkBuddyAi()} disabled={asking || !askQuery.trim()} className="size-7 rounded-md border border-border grid place-items-center shrink-0 disabled:opacity-40">
